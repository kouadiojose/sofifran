          ...
          <div class="col-md-4">
            <div class="card">
              <div class="card-header">
                <h2 class="card-title">Editer un role</h2>

              </div>
              <!-- /.card-header -->
              <div class="card-body">
                  
                  <form action="{{ route('admin-edit-valid-role') }}" method="post">

                    {{ csrf_field() }}
                    <input type="hidden" name="role_id" value="{{ $role->id }}">
                    <div class="form-group">
                      <label>Name</label>
                      <input type="text" name="name" class="form-control" placeholder="Admin" value="{{ $role->name }}" required="required">
                    </div>
                    <div class="form-group">
                      <label>Description</label>
                      <textarea rows="5" name="description" placeholder="Description..." class="form-control">{{ $role->description }}</textarea>
                    </div>
                  ...
                  <table class="table table-bordered" id="example1">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>{{ $r->description }}</td>
                        <td width="150">
                          <a href="{{ route('admin-edit-role', $r->id) }}" class="btn btn-secondary"><i class="fa fa-pencil-alt"></i></a>
                          <a href="javascript();" data-toggle="modal" data-target="#del_role" data-id="{{ $r->id }}" class="btn btn-danger"><i class="fa fa-trash"></i></a>                        
                        </td>
                      </tr>
                      @endforeach
                    </tbody>
