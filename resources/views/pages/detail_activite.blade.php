...
<?php
  if( app()->getLocale() == 'fr' ){
    $langue = 'Détail Activité';
  }else{
    $langue = 'Activity Detail';
  }
?>

@section('title', $langue)

@section('meta_description', \Illuminate\Support\Str::limit(strip_tags(app()->getLocale() == 'fr' ? $activite->description_fr : $activite->description_en), 160))
@section('meta_image', url('/frontend/assets/images/activites/' . $activite->image))
            ...
            <div class="d-table-cell">
                <div class="container">
                    <div class="page-title-content">
                        <h2>{{ app()->getLocale() == 'fr' ? 'Détail Activité': 'Activity Detail' }}</h2>
                        <ul>
                            <li><a href="{{ route('index') }}">{{ app()->getLocale() == 'fr' ? 'Accueil': 'Home' }}</a></li>
                            <li>{{ app()->getLocale() == 'fr' ? 'Détail Activité': 'Activity Detail' }}</li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="col-lg-8 col-md-12">
                    <div class="blog-details-desc">
                        <div class="article-image">
                            <img src="/frontend/assets/images/activites/{{ $activite->image }}" alt="image">
                        </div>

                        <div class="article-content">
                            <div class="entry-meta">
                                <ul>
                                    <li><i class="fi fi-br-clock"></i> <a href="#">{{ date('d M Y', strtotime($activite->created_at)) }}</a></li>
                                </ul>
                            </div>

                            <h3>{{ app()->getLocale() == 'fr' ? $activite->title_fr: $activite->title_en }}</h3>

                            <p>
                            	<?= app()->getLocale() == 'fr' ? $activite->description_fr: $activite->description_en; ?>
                            </p>

                            @if( isset($activite->link_img) )
                            <a target="_blank" class="default-btn" href="{{$activite->link_img}}">Voir les images de {{ app()->getLocale() == 'fr' ? $activite->title_fr: $activite->title_en }}</a>
                            @endif
                        </div>
                ...
                <div class="col-lg-4 col-md-12">
                    <aside class="widget-area" id="secondary">
                        <section class="widget widget_pearo_posts_thumb">
                            ...
                            @foreach( $activites as $b )
	                            <article class="item">
	                                <a href="{{ route('detail-activites', [$b->categorie_activity_slug, $b->slug]) }}" class="thumb">
	                                    <span class="fullimage cover" style="background: url('/frontend/assets/images/activites/{{ $b->image }}');" role="img"></span>
	                                </a>
	                                <div class="info">
	                                    <time datetime="{{ $b->created_at }}">{{ date('d M Y', strtotime($b->created_at)) }}</time>
	                                    <h4 class="title usmall"><a href="{{ route('detail-activites', [$b->categorie_activity_slug, $b->slug]) }}">{{ app()->getLocale() == 'fr' ? $b->title_fr: $b->title_en }}</a></h4>
	                                </div>
